...
func TestDecodeMetadataRespV1(t *testing.T) {
	bytes := testHex2Bytes(t, "00000002000000010000000100096c6f63616c686f737400002384ffff000000010000000100000005746f70696300000000010000000000000000000100000001000000010000000100000001")
	metadataResp, err := DecodeMetadataResp(bytes, 1)
	assert.Nil(t, err)
	...
	assert.Equal(t, int32(1), brokerMetadata.NodeId)
	...
	assert.Equal(t, int32(1), metadataResp.ControllerId)
	...
	assert.Equal(t, int32(1), partitionMetadata.LeaderId)
...
func TestDecodeAndEncodeMetadataRespV1(t *testing.T) {
	bytes := testHex2Bytes(t, "00000002000000010000000100096c6f63616c686f737400002384ffff000000010000000100000005746f70696300000000010000000000000000000100000001000000010000000100000001")
	metadataResp, err := DecodeMetadataResp(bytes, 1)
	assert.Nil(t, err)
	assert.Equal(t, 2, metadataResp.CorrelationId)
...
func TestDecodeMetadataRespV8(t *testing.T) {
	bytes := testHex2Bytes(t, "0000000100000000000000010000000100096c6f63616c686f737400002384ffff00167635346934394266536c656b356e357a7a2d4c634a4100000001000000010000000a746573742d746f7069630000000001000000000000000000010000000000000001000000010000000100000001000000008000000080000000")
	metadataResp, err := DecodeMetadataResp(bytes, 8)
	assert.Nil(t, err)
	assert.Equal(t, 1, metadataResp.CorrelationId)
	...
	assert.Equal(t, 9092, metadataResp.BrokerMetadataList[0].Port)
	assert.Nil(t, brokerMetadata.Rack)
	assert.Equal(t, "v54i49BfSlek5n5zz-LcJA", metadataResp.ClusterId)
	assert.Equal(t, int32(1), metadataResp.ControllerId)
	assert.Len(t, metadataResp.TopicMetadataList, 1)
	topicMetadata := metadataResp.TopicMetadataList[0]
	assert.Equal(t, ErrorCode(0), topicMetadata.ErrorCode)
...
func TestDecodeAndEncodeMetadataRespV8(t *testing.T) {
	bytes := testHex2Bytes(t, "0000000100000000000000010000000100096c6f63616c686f737400002384ffff00167635346934394266536c656b356e357a7a2d4c634a4100000001000000010000000a746573742d746f7069630000000001000000000000000000010000000000000001000000010000000100000001000000008000000080000000")
	metadataResp, err := DecodeMetadataResp(bytes, 8)
	assert.Nil(t, err)
	assert.Equal(t, 1, metadataResp.CorrelationId)
...
func TestDecodeAndEncodeMetadataRespV9(t *testing.T) {
	bytes := testHex2Bytes(t, "00000002000000000002000000000a6c6f63616c686f7374000023840000174b304345692d436152543231437064526879425241770000000002000007746573742d330002000000000000000000000000000002000000000200000000010080000000008000000000")
	metadataResp, err := DecodeMetadataResp(bytes, 9)
	assert.Nil(t, err)
	assert.Equal(t, 2, metadataResp.CorrelationId)
