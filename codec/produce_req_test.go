...
func TestEncodeProduceReqV8(t *testing.T) {
	...
	produceReq.ClientId = "ddab32c9-f620-40a2-afb8-18b76696be0d"
	produceReq.TransactionId = nil
	produceReq.RequiredAcks = 1
	produceReq.Timeout = 30_000
	produceReq.TopicReqList = make([]*ProduceTopicReq, 1)
	produceTopicReq := &ProduceTopicReq{Topic: "topic", PartitionReqList: make([]*ProducePartitionReq, 1)}
	records := make([]*Record, 1)
	msgBytes := []byte("value")
	records[0] = &Record{RecordAttributes: 0, RelativeOffset: 0, RelativeTimestamp: 0, Key: []byte("key"), Value: msgBytes, Headers: nil}
	recordBatch := &RecordBatch{Offset: 0, MessageSize: 64, LeaderEpoch: -1, MagicByte: 2, Flags: 0, LastOffsetDelta: 0,
		FirstTimestamp: 1642428117718, LastTimestamp: 1642428117718, ProducerId: -1, ProducerEpoch: -1,
		BaseSequence: -1, Records: records}
	producePartitionReq := &ProducePartitionReq{PartitionId: 0, RecordBatch: recordBatch}
	produceTopicReq.PartitionReqList[0] = producePartitionReq
	produceReq.TopicReqList[0] = produceTopicReq
...
func TestDecodeAndCodeProduceReqV8(t *testing.T) {
	bytes := testHex2Bytes(t, "")
	produceReq, err := DecodeProduceReq(bytes, 8)
	assert.Nil(t, err)
	assert.Equal(t, 4, produceReq.CorrelationId)
