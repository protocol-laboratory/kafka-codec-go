...
func TestEncodeRecordBatch64Bytes(t *testing.T) {
	r := &RecordBatch{}
	r.Offset = 0
	r.MessageSize = 64
	r.LeaderEpoch = 0
	...
	record.RelativeOffset = 0
	record.Key = nil
	record.Value = []byte(strings.Repeat("a", 64))
	r.Records[0] = record
	r.Bytes()
	assert.Equal(t, 134, r.BytesLength())
}

func TestEncodeRecordBatch128Bytes(t *testing.T) {
	r := &RecordBatch{}
	r.Offset = 0
	r.MessageSize = 64
	r.LeaderEpoch = 0
	...
	record.RelativeOffset = 0
	record.Key = nil
	record.Value = []byte(strings.Repeat("a", 128))
	r.Records[0] = record
	r.Bytes()
	assert.Equal(t, 198, r.BytesLength())
}
